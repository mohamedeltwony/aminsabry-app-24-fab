  ...
  Widget subscriptionEpisode(MovieData _episode) {
    if (data.userHasAccess.validate()) {
      return GestureDetector(
        ...
        child: SizedBox(
          height: !appStore.showPIP && !appStore.hasInFullScreen ? context.height() * 0.3 : context.height(),
          width: context.width(),
          child: Stack(
            ...
            children: [
              VideoContentWidget(
                ...
                fileLink: _episode.episodeFile.validate().isNotEmpty ? _episode.episodeFile.validate() : _episode.file.validate(),
      ...
      child: PiPBuilder(
        builder: (statusInfo) {
          ...
          return Observer(
            builder: (context) {
              return Scaffold(
                appBar: ((statusInfo?.status == PiPStatus.enabled) || (!data.userHasAccess.validate()) || appStore.hasInFullScreen || widget.watchedTime.isNotEmpty)
                    ? null
                    : AppBar(
                    ...
                    AnimatedScrollView(
                      ...
                      children: [
                        ...
                        if (statusInfo?.status != PiPStatus.enabled && !appStore.hasInFullScreen) ...[
                                ...
                                children: <Widget>[
                                  ...
                                  Row(
                                    ...
                                    children: [
                                      ...
                                      if (data.episodeFile.validate().isNotEmpty && data.userHasAccess.validate())
                                        Container(
                                          padding: EdgeInsets.all(12),
                                          decoration: BoxDecoration(color: cardColor, borderRadius: radius(4)),
                                      ...
                                      if (data.episodeFile.validate().isNotEmpty && appStore.isLogging && data.userHasAccess.validate())
                                            ...
                                            if (isIOS) {
                                              try {
                                                await MethodChannel("videoPlatform").invokeMethod('play', {
                                                  "data": data.episodeFile.validate().isNotEmpty ? data.episodeFile.validate() : data.urlLink.validate().replaceAll(r'\/', '/'),
                                                });
                                              ...
                                              FlPiP().enable(
                                                ios: FlPiPiOSConfig(
                                                  ...
                                                  enableControls: true,
                                                  packageName: null,
                                                  videoPath: data.episodeFile.validate(),
                                                  rect: Rect.fromLTRB(16, 16, 16, 16),
                                                ),
                          ...
                          if (data.sourcesList.validate().isNotEmpty && data.userHasAccess.validate())
                            Text(
                              language!.sources,
                              style: primaryTextStyle(size: 18),
                              maxLines: 1,
                          ...
                          ).visible(data.sourcesList.validate().isNotEmpty && data.userHasAccess.validate()),
                          if (showComments && data.isCommentOpen.validate() && data.userHasAccess.validate())
                            CommentWidget(
                              postId: data.id,
                          ...
                          headingWidViewAll(context, language!.episodes, showViewMore: false)
                              .paddingOnly(left: spacing_standard, right: spacing_standard)
