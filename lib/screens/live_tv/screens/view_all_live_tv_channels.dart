...
class ViewAllLiveTvChannels extends StatefulWidget {
  ...
  const ViewAllLiveTvChannels({Key? key, required this.categoryTitle, required this.sliderIndex}) : super(key: key);
...
class _ViewAllLiveTvChannelsState extends State<ViewAllLiveTvChannels> {
  ...
  Future<void> init({bool showLoader = true, int page = 1}) async {
    ...
    future = viewAll(widget.sliderIndex, dashboardTypeLive, page: page).then((value) {
          ...
          SnapHelperWidget(
            ...
            onSuccess: (data) {
              if (channelList.isEmpty)
                return NoDataWidget(
                  imageWidget: noDataImage(),
                  title: 'No channels found for ${widget.categoryTitle}',
                  onRetry: () {
                    setState(() {
                      mPage = 1;
                    });
